    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            if (!_roleManager.RoleExistsAsync(SD.Role_Admin).GetAwaiter().GetResult())
            {
                ...
                _roleManager.CreateAsync(new IdentityRole(SD.Role_Team_Lead)).GetAwaiter().GetResult();
                _roleManager.CreateAsync(new IdentityRole(SD.Role_Employee)).GetAwaiter().GetResult();
                _roleManager.CreateAsync(new IdentityRole(SD.Role_Quality_Assurance)).GetAwaiter().GetResult();
            }
        ...
        private Task_Management_SystemUser CreateUser()
        {
            try
            {
                return Activator.CreateInstance<Task_Management_SystemUser>();
            }
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(Task_Management_SystemUser)}'. " +
                    $"Ensure that '{nameof(Task_Management_SystemUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<Task_Management_SystemUser> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<Task_Management_SystemUser>)_userStore;
        }
    }
}
